...
#include "CodeUtils.h"
#include "CHIPVendorIdentifiers.hpp"
...
bool PayloadContents::isValidQRCodePayload() const
{
    ...
    if (!rendezvousInformation.has_value()) 
    {
        return false;
    }
...
{
    if (optionalVendorData.find(tag) != optionalVendorData.end()) {
...
{
    if (optionalExtensionData.find(kSerialNumberTag) != optionalExtensionData.end()) {
...
CHIP_ERROR SetupPayload::getOptionalExtensionData(uint8_t tag, OptionalQRCodeInfoExtension & info) const
{
    ...
    if (it != optionalExtensionData.end()) {
        info = it->second;
        return CHIP_NO_ERROR;
    }
