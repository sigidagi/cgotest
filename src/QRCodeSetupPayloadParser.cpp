...
static CHIP_ERROR readBits(std::vector<uint8_t> buf, size_t & index, uint64_t & dest, size_t numberOfBitsToRead)
{
    ...
    {
        fmt::print("Error parsing QR code. startIndex {} numberOfBitsToLoad {} buf_len {} ",
...
CHIP_ERROR QRCodeSetupPayloadParser::populatePayload(SetupPayload & outPayload)
{
    ...
    outPayload.rendezvousInformation.emplace(
    ...
    if (dest != 0)
    {
        fmt::print("Payload padding bits are not all 0: {0:x}", static_cast<unsigned>(dest));
        return CHIP_ERROR_INVALID_ARGUMENT;
    }
